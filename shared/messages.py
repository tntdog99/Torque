...
def X3DH_recv(prekey_priv, long_term_encrypttion_priv, otk_priv, initiator_long_term_encrypttion_pub, initiator_throw_pub):
    """
...
def check_sig(signature, signed_data, verifying_key: Ed25519PublicKey):
    try:
        verifying_key.verify(base64.urlsafe_b64decode(signature), base64.urlsafe_b64decode(signed_data))
...
def make_inner_message(contact_id, payload, compression_type="gzip", payload_type="text", part_number=1):
    inner_message = {
        ...
        "message_bytes": base64.urlsafe_b64encode(compress(payload.encode(), compression_type)).decode(),
        "type_of_key_or_message": "message"
    }
    Path(storage_path/"contacts"/contact_id/"messages"/'sent').mkdir(parents=True, exist_ok=True)
    msg_path = Path(storage_path/"contacts"/contact_id/"messages"/'sent'/f"{str(int(time.time()))}{str(uuid.uuid4())}.json")
    msg_path.write_text(json.dumps(inner_message))
    return inner_message

def consume_otk(key_id, contact_id):
    my_ident_pub, my_ident_priv = make_keys.grab_identify_keys()
...
def first_message_send_init(contact_id, message, sender_id):
    ...
    if otk_json == None:
        ...
        raise NoKeyFound("no otk key found on server")
    if prekey_bundle == None:
        logger.error("Empty semi_key payload for contact %s", contact_id)
        raise NoKeyFound("no pre key found on server")
    
    check_sig(prekey_bundle['long_term_encryption_pub_sig'], prekey_bundle['encrypt_pub'], verifying_key)
    ...
    encrypt_pub_bytes = lte_pub.public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)
    ...
    ratchet_state = ratchet.initialize_ratchet(contact_id, ratchet_pub, ratchet_priv, new_key, prekey_pub)
    outer_message = ratchet.ratchet_encrypt(ratchet_state, json.dumps(inner).encode(), contact_id, otk_json['key_id'], throw_pub, start=True, lte_pub=lte_pub, sender_id=sender_id, lte_pub_sig=long_term_encryption_pub_sig)
...
def first_message_recv_init(contact_id, msg, my_contact_id):
    ...
    header = full_header['ratchet_header']# dont really know why i made two headers, dont care enough to look into it, if its not broken dont fix it
    header_check = full_header['ratchet_header']
    
    nonce = base64.urlsafe_b64decode(msg['nonce'])
    
    lte_pub, lte_priv = make_keys.grab_long_term_encrypttion_keys()
    
    lte_other_pub = make_x25519_pub(msg['lte'])
    throw_pub = make_x25519_pub(msg["throw_pub"])
    
    prekey_path = contact_path/"semi_pub.json"
    prekey_bundle = json.loads(prekey_path.read_text())
    ...
    ratchet_state = ratchet.initialize_ratchet_recv(contact_id, new_pub, new_priv, new_key, prekey_pub)
...
def decode_message(contact_id, outer_message):
    ...
    contact_path = storage_path/"contacts"/contact_id
    ratchet_state = ratchet.load_ratchet(json.loads((contact_path/"ratchet_state.json").read_text()))
...
def encode_message(contact_id, message, sender_id):
    ...
    outer_message = ratchet.ratchet_encrypt(ratchet_state, json.dumps(inner).encode(), contact_id, None, None, sender_id=sender_id)
    ratchet_state.save(contact_id)
