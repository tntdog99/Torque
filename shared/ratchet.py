...
class ratchet_state_obj():
    ...
    ratchet_pub: Optional[X25519PublicKey]
    ratchet_priv: Optional[X25519PrivateKey]
    ratchet_pub_contact: Optional[X25519PublicKey]
    message_number: int
    recv_msg_number: int
    last_chain_length: int
    skipped_message_keys: Dict[str, bytes]

    def __init__(self, root_key, send_chain_key, recv_chain_key, ratchet_pub=None, ratchet_priv=None, ratchet_pub_contact=None, message_number=0,recv_msg_number=0, last_chain_length=0, skipped_message_keys=None):
        skipped_message_keys = skipped_message_keys if skipped_message_keys != None else {}
    ...
    def export(self):
        return {
            ...
            "send_chain_key": base64.urlsafe_b64encode(self.send_chain_key).decode() if self.send_chain_key else None,
            "recv_chain_key": base64.urlsafe_b64encode(self.recv_chain_key).decode() if self.recv_chain_key else None,
            "ratchet_pub": base64.urlsafe_b64encode(self.ratchet_pub.public_bytes(encoding=serialization.Encoding.Raw, format=serialization.PublicFormat.Raw)).decode() if self.ratchet_pub else None,
            "ratchet_priv": base64.urlsafe_b64encode(self.ratchet_priv.private_bytes(encoding=serialization.Encoding.Raw, format=serialization.PrivateFormat.Raw, encryption_algorithm=serialization.NoEncryption())).decode() if self.ratchet_priv else None,
            "ratchet_pub_contact": base64.urlsafe_b64encode(self.ratchet_pub_contact.public_bytes(encoding=serialization.Encoding.Raw, format=serialization.PublicFormat.Raw)).decode() if self.ratchet_pub_contact else None,
            "message_number": self.message_number,
            "recv_msg_number": self.recv_msg_number,
            "last_chain_length": self.last_chain_length,
            "skipped_message_keys": {k: base64.urlsafe_b64encode(v).decode() for k, v in self.skipped_message_keys.items()}
        }
...
def load_ratchet(exported):
    ...
    send_chain_key = base64.urlsafe_b64decode(exported["send_chain_key"]) if exported["send_chain_key"] else None
    recv_chain_key = base64.urlsafe_b64decode(exported["recv_chain_key"]) if exported["recv_chain_key"] else None
    ratchet_pub = X25519PublicKey.from_public_bytes(base64.urlsafe_b64decode(exported["ratchet_pub"])) if exported["ratchet_pub"] else None
    ratchet_priv = X25519PrivateKey.from_private_bytes(base64.urlsafe_b64decode(exported["ratchet_priv"])) if exported["ratchet_priv"] else None
    ratchet_pub_contact = X25519PublicKey.from_public_bytes(base64.urlsafe_b64decode(exported["ratchet_pub_contact"])) if exported["ratchet_pub_contact"] else None
    ...
    return ratchet_state_obj(root_key, send_chain_key, recv_chain_key, ratchet_pub, ratchet_priv, ratchet_pub_contact, message_number, recv_msg_number, last_chain_length, skipped_message_keys)
...
def initialize_ratchet(contact_id, ratchet_pub, ratchet_priv, new_key, prekey_pub):
    ...
    ratchet_priv = ratchet_priv
    ratchet_pub_contact = prekey_pub
    message_number = 0
    recv_msg_number = 0
    last_chain_length = 0
    skipped_message_keys = {}
    ratchet_state = ratchet_state_obj(root_key, send_chain_key, recv_chain_key, ratchet_pub, ratchet_priv, ratchet_pub_contact, message_number, recv_msg_number, last_chain_length, skipped_message_keys)
...
def initialize_ratchet_recv(contact_id, ratchet_pub, ratchet_priv, new_key, prekey_pub):
...
def ratchet_encrypt(ratchet_state: ratchet_state_obj, msg, contact_id, otk_id, throw_pub=None, start=False, lte_pub=None, sender_id=None, lte_pub_sig=None):
    ...
    outer_message = {
        ...
        "throw_pub": base64.urlsafe_b64encode(throw_pub.public_bytes(encoding=serialization.Encoding.Raw, format=serialization.PublicFormat.Raw)).decode() if throw_pub else None,
        "start": start,
        "lte_sig": lte_pub_sig,
        "lte": base64.urlsafe_b64encode(lte_pub.public_bytes(encoding=serialization.Encoding.Raw, format=serialization.PublicFormat.Raw)).decode() if lte_pub else None,
    }
    ratchet_state.message_number += 1
    logger.debug("Encrypted ratchet message for contact %s message number %s", contact_id, ratchet_state.message_number)
...
def ratchet_decrypt(ratchet_state, outer_message):
    ...
    skipped_key = ratchet_state.skipped_message_keys.get(f"{incoming_ratchet_pub}:{header['message_number']}")
    if skipped_key:
    ...
    if incoming_ratchet_pub != None and incoming_ratchet_pub != current_contact_pub:
        logger.debug("Performing DH ratchet for new remote ratchet pub")
        if header['last_chain_length'] > 1000:
                logger.warning("Incoming last_chain_length %s exceeds safety threshold", header['last_chain_length'])
                return None
        ...
        ratchet_state.root_key, ratchet_state.recv_chain_key = kdf_root(ratchet_state.root_key, dh_out)
        ...
        ratchet_state.root_key, ratchet_state.send_chain_key = kdf_root(ratchet_state.root_key, dh_out2)
    ...
    if header['message_number'] > ratchet_state.recv_msg_number:
        if header['message_number'] > 1000:
            logger.warning("Incoming message_number %s exceeds safety threshold", header['message_number'])
            return None
        logger.debug("Skipping %s intermediate receive messages", header['message_number'] - ratchet_state.recv_msg_number)
    ...
    logger.debug("Decrypted message %s, updated recv_msg_number to %s", header['message_number'], ratchet_state.recv_msg_number)
